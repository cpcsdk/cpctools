...
int IMG_SavePNG_RW(SDL_RWops *src, SDL_Surface *surf,int compression){
	...
	if (fmt->BitsPerPixel==8){ /* Paletted */
		...
		if (surf->flags&SDL_SRCCOLORKEY) {
			...
			png_set_tRNS(png_ptr, info_ptr, palette_alpha, fmt->colorkey + 1,
				NULL);
		}
	}else{ /* Truecolor */
		if (fmt->Amask) {
	...
	if (fmt->BitsPerPixel==8) { /* Paletted */
		...
		if (funky_format) {
			...
			if (fmt->Amask){
#if SDL_BYTEORDER == SDL_BIG_ENDIAN
				tempsurf = SDL_CreateRGBSurface(SDL_SWSURFACE, surf->w, surf->h,
					24,	0xff000000, 0x00ff0000, 0x0000ff00, 0x000000ff);
#else
				tempsurf = SDL_CreateRGBSurface(SDL_SWSURFACE, surf->w, surf->h,
					24,	0x000000ff, 0x0000ff00, 0x00ff0000, 0xff000000);
#endif
			}else{
#if SDL_BYTEORDER == SDL_BIG_ENDIAN
				tempsurf = SDL_CreateRGBSurface(SDL_SWSURFACE, surf->w, surf->h,
					24,	0xff0000, 0x00ff00, 0x0000ff, 0x00000000);
#else
				tempsurf = SDL_CreateRGBSurface(SDL_SWSURFACE, surf->w, surf->h,
			...
			if (used_alpha) {
				...
				SDL_SetAlpha(surf, SDL_SRCALPHA,(Uint8)temp_alpha);
			}

			for (int i = 0; i<tempsurf->h; i++){
				row_pointers[i]= ((png_byte*)tempsurf->pixels)
					+ i * tempsurf->pitch;
			}
