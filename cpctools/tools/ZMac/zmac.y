...
%type <ival> reg hiddenreg common8bitreg evenreg realreg mem index pushable bcdesp bcdehlsp mar condition
	...
	SHIFT reg ',' reg
		{
			...
			emit1(0145400 + ($1->i_value << 3) + ($4 & 0377), $2, 0, 0);
		}
|
	INCDEC reg
...
|
	RES expression ',' index ',' common8bitreg
		{
			if ($2 < 0 || $2 > 7)
				err[vflag]++;
			emit1((($1->i_value + (($2 & 7) << 3) + ($4 & 0377)) & 0x0FFF8) + ($6 & 7), $4, 0, 0);
		}
|
...
|
	LD reg ',' reg
		{
			if ((($2 & 0377) == 6 && ($4 & 0377) == 6)||((($2>>8)^($4>>8))==0x020)||(($2>=0x0DD00)&&($4>=0x0DD00)&&((($2&07)|($4&07))>=6))) {
				if (!pass2)
					fprintf(stderr,"LD reg, reg error\n");
				err[fflag]++;
			}
			emit1(0100 + (($2 & 7) << 3) + ($4 & 7),($2>$4?$2:$4), 0, 0);
...
|
	hiddenreg
;
common8bitreg: /* a,b,c,d,e,h,l,ixh,ixl, iyh, iyl */
	realreg
|
	hiddenreg
;
