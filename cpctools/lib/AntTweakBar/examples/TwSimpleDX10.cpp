...
int WINAPI WinMain(HINSTANCE instance, HINSTANCE, LPSTR, int cmdShow)
{
	...
	WNDCLASSEX wcex = { sizeof(WNDCLASSEX), CS_HREDRAW|CS_VREDRAW, MessageProc, 0L, 0L, instance, NULL, NULL, NULL, NULL, L"TwDX10", NULL };
	RegisterClassEx(&wcex);

	// Create a window
	RECT rc = { 0, 0, 640, 480 };
    AdjustWindowRect(&rc, WS_OVERLAPPEDWINDOW, FALSE);
	HWND wnd = CreateWindow(L"TwDX10", L"AntTweakBar simple example using DirectX10", WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, rc.right-rc.left, rc.bottom-rc.top, NULL, NULL, instance, NULL);
	if( !wnd )
	{
	...
	TwAddVarRW(bar, "Rotation", TW_TYPE_INT32, &g_Angle, " KeyIncr=r KeyDecr=R Help='Rotates the triangle (angle in degree).' ");
	TwAddVarRW(bar, "Scale", TW_TYPE_FLOAT, &g_Scale, " Min=-2 Max=2 Step=0.01 KeyIncr=s KeyDecr=S Help='Scales the triangle (1=original size).' ");
...
HRESULT InitDevice(HWND wnd)
{
	...
	hr = D3D10CreateDeviceAndSwapChain(NULL, D3D10_DRIVER_TYPE_HARDWARE, NULL, createDeviceFlags, D3D10_SDK_VERSION, &g_SwapChainDesc, &g_SwapChain, &g_D3DDevice);
	if( FAILED(hr) )
	{
		// If fail, try to create a reference device
		hr = D3D10CreateDeviceAndSwapChain(NULL, D3D10_DRIVER_TYPE_REFERENCE, NULL, createDeviceFlags, D3D10_SDK_VERSION, &g_SwapChainDesc, &g_SwapChain, &g_D3DDevice);
		if( SUCCEEDED(hr) )
...
HRESULT InitScene()
{
	...
	char shaderFX[] = " struct PSInput { float4 Pos : SV_POSITION; float4 Col : COLOR0; }; \n"
					  " PSInput VertShad(float4 pos : POSITION, float4 col : COLOR) { PSInput ps; ps.Pos=pos; ps.Col=col; return ps; } \n"
	...
	hr = D3D10CompileEffectFromMemory(shaderFX, strlen(shaderFX), "TestDX10", NULL, NULL, shaderFlags, 0, &compiledFX, &errors);
	if( FAILED(hr) )
	{
	...
	hr = D3D10CreateEffectFromMemory(compiledFX->GetBufferPointer(), compiledFX->GetBufferSize(), 0, g_D3DDevice, NULL, &g_Effect);
	compiledFX->Release();
    ...
    hr = g_D3DDevice->CreateInputLayout(layout, sizeof(layout)/sizeof(layout[0]), passDesc.pIAInputSignature, passDesc.IAInputSignatureSize, &g_VertexLayout);
    if( FAILED( hr ) )
        return hr;
		...
		case WM_SIZE:	// Window size has been changed
			if( g_D3DDevice )	// Resize D3D render target
			{
				...
				if( g_SwapChain )
				{
					...
					g_SwapChain->ResizeBuffers(g_SwapChainDesc.BufferCount, g_SwapChainDesc.BufferDesc.Width, g_SwapChainDesc.BufferDesc.Height, g_SwapChainDesc.BufferDesc.Format, g_SwapChainDesc.Flags);
