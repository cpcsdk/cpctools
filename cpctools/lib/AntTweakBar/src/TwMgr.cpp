...
void ANT_CALL CQuaternionExt::InitCB(void *_ExtValue, void *_ClientData)
{
...
void ANT_CALL CQuaternionExt::SummaryCB(char *_SummaryString, size_t _SummaryMaxLength, const void *_ExtValue, void * /*_ClientData*/)
{
	...
	if( ext )
	{
		_snprintf(_SummaryString, _SummaryMaxLength, "X:%.2g Y:%.2g Z:%.2g S:%.2g", ext->Qx, ext->Qy, ext->Qz, ext->Qs);
	}
	else
	{
		_SummaryString[0] = ' ';	// required to force background color for this value
...
void CQuaternionExt::CreateType()
{
	if( g_TwMgr==NULL )
		return;
	TwStructMember QuatExtMembers[] = { { "1", TW_TYPE_CUSTOM, 0, "" },
										{ "2", TW_TYPE_CUSTOM, 0, "" },
										{ "3", TW_TYPE_CUSTOM, 0, "" }, 
										{ "X", TW_TYPE_FLOAT, offsetof(CQuaternionExt, Qx), "" },
										{ "Y", TW_TYPE_FLOAT, offsetof(CQuaternionExt, Qy), "" },
										{ "Z", TW_TYPE_FLOAT, offsetof(CQuaternionExt, Qz), "" },
										{ "S", TW_TYPE_FLOAT, offsetof(CQuaternionExt, Qs), "" } };
	g_TwMgr->m_TypeQuaternion = TwDefineStructExt("QUATERNION", QuatExtMembers, sizeof(QuatExtMembers)/sizeof(QuatExtMembers[0]), 4*sizeof(float), sizeof(CQuaternionExt), CQuaternionExt::InitCB, CQuaternionExt::CopyVarFromExtCB, CQuaternionExt::CopyVarToExtCB, CQuaternionExt::SummaryCB, CTwMgr::CStruct::s_PassProxyAsClientData, NULL);
}
...
void CTwMgr::UnrollCDStdString(std::vector<CCDStdStringRecord>& _Records, TwType _Type, void *_Data)
{
	if( _Data!=NULL )
	{
		...
		else if( _Type==TW_TYPE_STDSTRING || _Type==TW_TYPE_CDSTDSTRING )
		{
			...
			Rec.m_DataPtr = _Data;
			memcpy(Rec.m_PrevValue, _Data, sizeof(std::string));
			const char *Str = *(const char **)_Data;
			if( Str!=NULL )
				Rec.m_StdString = Str;
			memcpy(_Data, &Rec.m_StdString, sizeof(std::string));
		}
	}
}
...
void CTwMgr::RestoreCDStdString(const std::vector<CCDStdStringRecord>& _Records)
{
	for( size_t i=0; i<_Records.size(); ++i )
		memcpy(_Records[i].m_DataPtr, _Records[i].m_PrevValue, sizeof(std::string));
}
...
void ANT_CALL CTwMgr::CCDStdString::SetCB(const void *_Value, void *_ClientData)
{
	...
	if( CDStdString->m_ClientSetCallback==NULL )
	{
		if( g_TwMgr->m_CopyStdStringToClient && CDStdString->m_ClientStdStringPtr!=NULL )
			g_TwMgr->m_CopyStdStringToClient(*(CDStdString->m_ClientStdStringPtr), SrcStr);
	}
	else
	{
...
static int AddVar(TwBar *_Bar, const char *_Name, ETwType _Type, void *_VarPtr, bool _ReadOnly, TwSetVarCallback _SetCallback, TwGetVarCallback _GetCallback, TwButtonCallback _ButtonCallback, void *_ClientData, const char *_Def)
{
	...
	//if( _Type==TW_TYPE_QUATERNION )
	//	_Type = g_TwMgr->m_TypeQuaternion;
		...
		else
		{
			...
			if( _Type==TW_TYPE_BUTTON )
			{
				...
				if( _ButtonCallback==NULL && _ClientData==&s_SeparatorTag )
				{
					...
					Var->m_Color = _Bar->m_ColStaticText;
			}
			if( _Type!=TW_TYPE_BUTTON )
...
void ANT_CALL TwCopyStdStringToLibrary(std::string& destLibraryString, const std::string& srcClientString)
{
	...
	*/
	...
	const char *SrcStr = srcClientString.c_str();
