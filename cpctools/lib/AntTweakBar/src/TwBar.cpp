...
void CTwBar::UpdateColors()
{
	...
	ColorHLSToRGBf(h, 0.15f, s, &r, &g, &b);
	m_ColTitleHighBg = Color32FromARGBf(a, r, g, b);
	ColorHLSToRGBf(h, 0.3f, s, &r, &g, &b);
	m_ColTitleUnactiveBg = Color32FromARGBf(a, r, g, b);
	...
	ColorHLSToRGBf(h, 0.1f, s, &r, &g, &b);
	m_ColHighFold = Color32FromARGBf(0.3f*a, r, g, b);
	...
	m_ColEditText = COLOR32_BLACK;
	m_ColEditBg = COLOR32_WHITE; //0xff000000 | m_ColValBg;
	m_ColEditSelText = COLOR32_WHITE;
	m_ColEditSelBg = 0xff000000 | m_ColBtn;
...
void CTwBar::ListLabels(vector<string>& _Labels, vector<color32>& _Colors, vector<color32>& _BgColors, bool *_HasBgColors, const CTexFont *_Font, int _AtomWidthMax, int _GroupWidthMax)
{
	...
	for( int h=0; h<nh; ++h )
	{
		...
		color32 bg = m_HierTags[h].m_Var->m_BgColor;
		_BgColors.push_back(bg);
		if( _HasBgColors!=NULL && bg!=0 )
			*_HasBgColors = true;
		bool IsCustom = !m_HierTags[h].m_Var->IsGroup() && static_cast<const CTwVarAtom *>(m_HierTags[h].m_Var)->m_Type==TW_TYPE_CUSTOM;
		if( !IsCustom )
		{
			...
			|| (m_HierTags[h].m_Var->IsGroup() && static_cast<const CTwVarGroup *>(m_HierTags[h].m_Var)->m_SummaryCallback!=NULL) )
		{
			...
			if( !m_HierTags[h].m_Var->IsGroup() )
			{
				...
				if(    (Atom->m_Type==TW_TYPE_CDSTRING && Atom->m_SetCallback==NULL && g_TwMgr->m_CopyCDStringToClient==NULL)
					...
					IsROText = true;
				if( Atom->m_Type==TW_TYPE_HELP_ATOM || Atom->m_Type==TW_TYPE_HELP_GRP || Atom->m_Type==TW_TYPE_BUTTON || Atom->m_Type==TW_TYPE_CUSTOM )
					HasBgColor = false;
...
void CTwBar::Draw()
{
	...
	if( !m_IsMinimized )
	{
		...
		{
			PERF( Timer.Reset(); )
			Gr->DrawRect(m_PosX, m_PosY, m_PosX+m_Width-1, m_PosY+m_Font->m_CharHeight, (m_HighlightTitle||m_MouseDragTitle) ? m_ColTitleHighBg : (m_DrawHandles ? m_ColTitleBg : m_ColTitleUnactiveBg));
		...
		if( m_HighlightedLine>=0 && m_HighlightedLine<(int)m_HierTags.size() && m_HierTags[m_HighlightedLine].m_Var!=NULL
			&& (m_HierTags[m_HighlightedLine].m_Var->IsGroup() || (!static_cast<CTwVarAtom *>(m_HierTags[m_HighlightedLine].m_Var)->m_ReadOnly && !m_IsHelpBar && static_cast<CTwVarAtom *>(m_HierTags[m_HighlightedLine].m_Var)->m_Type!=TW_TYPE_CUSTOM)) )
			{
				int y0 = m_PosY + m_VarY0 + m_HighlightedLine*(m_Font->m_CharHeight+m_Sep);
				Gr->DrawRect(m_PosX+LevelSpace+6+LevelSpace*m_HierTags[m_HighlightedLine].m_Level, y0, m_PosX+m_VarX2, y0+m_Font->m_CharHeight-1, m_ColHighBg);
		...
		if( m_DrawHandles && !m_IsPopupList )
		{
			...
			if( (m_DrawIncrDecrBtn || m_DrawClickBtn || m_DrawListBtn || m_DrawBoolBtn || m_DrawRotoBtn) && m_HighlightedLine>=0 && m_HighlightedLine<(int)m_HierTags.size() )
			{
				...
				else if( m_DrawListBtn )
				{
					...
					Gr->DrawLine(m_PosX+m_VarX2-bw+4+dx, y0+m_Font->m_CharHeight/2-1, m_PosX+m_VarX2-bw/2+1+dx, y0+m_Font->m_CharHeight-4, m_ColTitleText, true);
	...
	if( !m_IsMinimized )
	{
		...
		else if( m_IsHelpBar && _Pressed && !g_TwMgr->m_IsRepeatingMousePressed && _X>=m_PosX+m_VarX0 && _X<m_PosX+m_Width-m_Font->m_CharHeight && _Y>m_PosY+m_Height-m_Font->m_CharHeight && _Y<m_PosY+m_Height )
		{
			const char *WebPage = "http://www.antisphere.com/Wiki/tools:anttweakbar";
				...
				// brut force: try all the possible browsers (I don't know how to find the default one; someone?)
				char DefaultBrowsers[] = "mozilla,firefox,netscape,konqueror,galeon,opera,dillo";
