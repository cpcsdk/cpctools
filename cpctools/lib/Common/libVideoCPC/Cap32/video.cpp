...
SDL_Surface* HalfSizeHardwarePlugin::Init(int w,int h, int bpp, bool fs)
{
	...
	_video=SDL_SetVideoMode(CPCVisibleSCRWidth,CPCVisibleSCRHeight,bpp,SDL_ANYFORMAT | SDL_DOUBLEBUF | SDL_HWSURFACE | SDL_HWPALETTE | (fs?SDL_FULLSCREEN:0));
	if (!_video)
		return NULL;
	SDL_FillRect(_video,NULL,SDL_MapRGB(_video->format,0,0,0));
	return _video;
...
void SuperEaglePlugin::Filter(	Uint8 *srcPtr, Uint32 srcPitch, /* Uint8 *deltaPtr,  */
								Uint8 *dstPtr, Uint32 dstPitch, int width, int height)
{
    Uint8  *dP;
    Uint16 *bP;
