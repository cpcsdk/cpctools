...
ymu32 n1,n2;
		...
		{
			a0 = pDataStream;
			ymint size = ymTrackerLine_sizeof*nbVoice*nbFrame;
			pNewBuffer = (unsigned char*)malloc(size);
			step = ymTrackerLine_sizeof*nbVoice;
			n1 = step;
			a2 = pNewBuffer;
			do
			{
...
//		pLine += (currentFrame*nbVoice);
		pLine = pDataStream+(currentFrame*nbVoice)*ymTrackerLine_sizeof;
	  	for (i=0;i<nbVoice;i++)
		{
