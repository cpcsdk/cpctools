...
def crea_track_info_block(num_pista, pcw):
    """
    ...
    tib_tmp += b'\x00' * 2                  # 12 - 13   unused              2
    ...
    num_sector_tmp = (0x01 if pcw else 0xC1)
    last_sector = (0x05 if pcw else 0xC5)
...
def procesar_linea_comandos(linea_de_comandos):
    ...
    descripcion_programa = "%prog insert files in sectors of a disk image for the Amstrad CPC and PCW."

    # definimos las opciones que soportaremos desde la línea de comandos
    lista_de_opciones = [
        make_option("-p", "--pcw", action="store_true", dest="pcw", default=False, help="Make a PCW disk image"),
...
#        make_option("-l", "--disk_loader", action="store", type="string", dest="disk_loader", default="", help="The file with the disk loader"),
        make_option("-l", "--logfile", action="store", type="string", dest="log_filename", default="", help="Dump the disk creation info"),
...
def main(linea_de_comandos=None):
    ...
    # Comprobamos si hay que crear un fichero con los datos del disco
    MAKE_LOG = (opciones.log_filename != "")
    fichero_de_log = ""
    LOG_LABEL = ""
    
    # Creamos un disco formateado
    disco = [ [crea_track_info_block(num_pista, opciones.pcw), \
    ...
    if not (opciones.format_disk or not(os.path.exists(opciones.disk_filename))):
        ...
        # Eliminamos los TIB
        disco_tmp = b"".join([disco_tmp [i] [LEN_TRACK_INFO_BLOCK: ] for i in range(len(disco_tmp))])
    ...
    if lista_ficheros != []:
        ...
        num_pista = opciones.disk_sector // 8
        num_sector = opciones.disk_sector % 8      # Sector inicial
        for nombre_fichero in lista_ficheros:
            ...
            if MAKE_LOG:
                fichero_de_log += "; " + nombre_fichero + "\n"
                LOG_LABEL = nombre_fichero.upper()[ : len(nombre_fichero) - 4]
...
#                fichero_en_sectores = particiona_fichero(fichero.read())
            ...
            if num_sector > 8:
                num_sector = 0
                num_pista += 1
            print ("Inicial --> Pista: %i Sector: %x" % (num_pista, 0xC1 + num_sector))
            # Añadimos información al fichero de log
            if MAKE_LOG:
                fichero_de_log += LOG_LABEL + "_BEGIN_TRACK EQU " + str(num_pista) + "\n"
                fichero_de_log += LOG_LABEL + "_BEGIN_SECTOR EQU " + str(0xC1 + num_sector) + \
                                " ; " + hex(0xC1 + num_sector) + "\n"
...
#                print ("Pista: %i Sector: %i" % (num_pista, num_sector))
                ...
                num_sector += 1
                sectores_totales += 1
            print ("Final --> Pista: %i Sector: %x Total: %i" % (num_pista, 0xC1 + num_sector - 1, sectores_totales))
            # Añadimos información al fichero de log
            if MAKE_LOG:
                fichero_de_log += LOG_LABEL + "_NUM_SECTORS EQU " + str(sectores_totales) + "\n\n"
    ...
    # Guardamos el fichero de log en disco
    if MAKE_LOG:
        with open(opciones.log_filename,"w") as fichero:
            fichero.write(fichero_de_log)
