...
static unsigned char sound_ay_current_registers[16];
...
if((sound_buf=malloc(sizeof(signed short)*sound_framesiz*(sound_stereo+1)*2))==NULL)    // TODO : add a good way to allocate variable size of buffer
  {
  sound_end();
  return(0);
  }
...
static void sound_ay_overlay(int nbSample)
{
static int rng=1;
...
for(f=0,ptr=sound_buf;f<nbSample;f++)
  {
...
if(ay_change_count<AY_CHANGE_MAX)
  ...
  sound_ay_current_registers[reg]=val;
}

int sound_ay_read(int reg)
{
    return sound_ay_current_registers[reg];
}
...
sound_ay_overlay(sound_framesiz);
...
/* returns zero if this frame was completely silent */
int sound_frameExt(int nbSample, int really_play)
{
static int silent_level=-1;
signed short *ptr;
int f,silent,chk;
int fulllen=nbSample*(sound_stereo+1);

ptr=sound_buf+(sound_stereo?sound_fillpos*2:sound_fillpos);
for(f=sound_fillpos;f<nbSample;f++)
  SOUND_WRITE_BUF_BEEPER(ptr,sound_oldval);

sound_ay_overlay(nbSample);
